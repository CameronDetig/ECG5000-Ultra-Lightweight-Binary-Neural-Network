...
def train(model: torch.nn.Module,
    ...
    best_test_epoch = 0
    best_train_epoch = 0
    best_test_model_path = None  # To store the path of the best train accuracy model
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if train_acc > best_train_acc:
            ...
            acc_str = "{:.2f}".format(best_train_acc * 100) + '%'
        ...
        if test_acc > best_test_acc:
            ...
            # Ensure the directory exists to save backpocket models
            output_dir = f"./saved_models/"
            os.makedirs(output_dir, exist_ok=True)

            # Save best weights to be loaded back onto model at the end of training
            best_test_model_path = f"{output_dir}best_test_model_epoch_{best_test_epoch}_{acc_str}.pth"
    ...
    # Load the model with the best training accuracy
    if best_test_model_path:
        print(f"\nLoading the model with the best test (validation) accuracy, {best_test_acc} from epoch {best_test_epoch}...")
